/*
  PDF-to-Word extraction is another long-running upload plus polling flow, so
  the composable keeps OCR option state and task progress typed before handing
  the request to the shared PDF service layer.
*/
...
type ExtractionLanguageMode = "en" | "gr" | "both";
type ExtractionQualityMode = "fast" | "quality" | "maximum" | "ultra";
...
export const usePdfExtractToWord = () => {
  ...
  const languageMode = ref<ExtractionLanguageMode>("both");
  const extractionQualityMode = ref<ExtractionQualityMode>("ultra");
  ...
  const extract = async (baseUrl: string): Promise<void> => {
    clearResult();
    error.value = "";
    resetProgress();
