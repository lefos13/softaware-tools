...
interface JsonVisualOptions {
  theme?: ThemeKey;
  fontSize?: number | string;
  title?: string;
}

interface JsonVisualRequest {
  toolId: string;
  input: string;
...
const wrapLine = ({
  ...
  const chunkToLines = (chunk: string): string[] => {
...
export const renderJsonAsImage = async ({
  toolId,
  input,
  options = {},
}: JsonVisualRequest): Promise<JsonVisualResult> => {
  const parsed = parseJsonInput(input) as unknown;
  const jsonText = JSON.stringify(parsed, null, 2);
  const theme = themeStyles[options.theme || "light"] || themeStyles.light;
