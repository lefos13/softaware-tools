...
import type { JsonToolOptions, JsonValue } from "../../../types/jsonTools";
import { parseJsonInput, formatPrettyJson, countObjects, stableStringify } from "./helpers";
import { ApiError } from "./types";

export const filterJson = (input: string, options: JsonToolOptions): string => {
...
export const countJsonObjects = (input: string): string => {
  const parsed = parseJsonInput(input);
...
export const findDuplicateJsonObjects = (input: string, options: JsonToolOptions): string => {
  const parsed = parseJsonInput(input);
  ...
  const map = new Map<string, { item: JsonValue; indices: number[] }>();
