...
export const convertJsonToYaml = (input: string, options: JsonToolOptions): string => {
...
export const convertYamlToJson = (input: string, options: JsonToolOptions): string => {
  try {
    const parsed = yaml.load(input) as JsonValue;
...
export const convertJsonToXml = (input: string, options: JsonToolOptions): string => {
  const parsed = parseJsonInput(input);
...
export const convertXmlToJson = (input: string, options: JsonToolOptions): string => {
  try {
    ...
    const parsed = parser.parse(input) as JsonValue;
    return formatPrettyJson(parsed, Number(options.indent || 2));
  } catch (error) {
...
export const convertJsonToCsv = (input: string): string => {
  ...
  */
  const toCsvCellValue = (value: JsonValue | undefined): string | number | boolean | null => {
    if (value === undefined) {
      return "";
    }
...
export const convertCsvToJson = (input: string, options: JsonToolOptions): string => {
  ...
  if (!hasHeader) {
    const rows = (result.data as string[][]).map((row) => row.map((cell) => parseCsvValue(cell)));
    return formatPrettyJson(rows, Number(options.indent || 2));
  }

  const rows = (result.data as Record<string, string>[]).map((row) => {
    const normalized: Record<string, ReturnType<typeof parseCsvValue>> = {};
...
const quoteSql = (value: JsonValue | undefined): string => {
...
export const convertJsonToSqlInsert = (input: string, options: JsonToolOptions): string => {
  const parsed = parseJsonInput(input);
  const rows = Array.isArray(parsed) ? parsed : [parsed];
  ...
  const valuesBlock = rows
    .map((row) => {
      const recordRow =
        row && typeof row === "object" && !Array.isArray(row) ? (row as JsonObject) : null;
      const values = keys.map((key) => quoteSql(recordRow?.[key] ?? null));
      return `(${values.join(", ")})`;
    })
    .join(",\n");
...
const splitSqlValues = (segment: string): string[] => {
  const values: string[] = [];
  let current = "";
  let inQuote = false;
...
const parseSqlLiteral = (literal: string): JsonValue => {
...
export const convertSqlInsertToJson = (input: string, options: JsonToolOptions): string => {
...
export const convertJsonToSqlQuery = (input: string, options: JsonToolOptions): string => {
...
export const convertJsonToBase64 = (input: string): string => {
  const parsed = parseJsonInput(input);
  return btoa(unescape(encodeURIComponent(formatMinifiedJson(parsed))));
};

export const convertBase64ToJson = (input: string, options: JsonToolOptions): string => {
  try {
...
export const convertJsonToBson = (input: string): string => {
  const parsed = parseJsonInput(input);
  const bytes = BSON.serialize(parsed as BSON.Document);
  return bytesToBase64(bytes);
};

export const convertBsonToJson = (input: string, options: JsonToolOptions): string => {
