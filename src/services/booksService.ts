...
import type {
  BinaryServiceResult,
  GreekEditorAccessResult,
  GreekEditorPreviewResult,
  GreekEditorTextResult,
  ServiceRequestOptions,
} from "../types/services";

const buildTaskSuffix = (taskId?: string, flowSessionId?: string): string => {
...
const buildServiceTokenHeaders = (serviceToken?: string): Record<string, string> =>
...
*/
export const validateGreekLiteratureEditorAccess = async (
  baseUrl: string,
  serviceToken?: string
): Promise<GreekEditorAccessResult> => {
  ...
  const payload = (await response.json()) as {
    data?: {
      authEnabled?: boolean;
      token?: GreekEditorAccessResult["token"];
    };
    message?: string;
    meta?: {
      requestId?: string;
...
export const previewGreekLiteratureEditorReport = async (
  ...
  editorOptions: Record<string, unknown> = {},
  options: ServiceRequestOptions = {}
): Promise<GreekEditorPreviewResult> => {
  const formData = new FormData();
  formData.append("files", file);
  formData.append("editorOptions", JSON.stringify(editorOptions || {}));
