...
const localizeJsonInputLabel = (label, locale) =>
...
const resolveMessage = (locale, path) =>
  path.split(".").reduce((value, segment) => value?.[segment], messages[locale]);
...
export const usePortalI18n = () => inject(PORTAL_I18N_KEY);

const buildOptionLabels = (field, t) =>
  (field.options || []).map((item) => ({
    ...item,
    label: t(`json.optionValues.${field.key}.${item.value}`, {}, item.label),
  }));

const localizeToolDefaultValue = (field, locale) => {
...
export const localizeJsonTool = (tool, { t, locale }) => ({
  ...tool,
  ...
  inputLabel: localizeJsonInputLabel(tool.inputLabel, locale),
  secondaryInputLabel: localizeJsonInputLabel(tool.secondaryInputLabel, locale),
